...
В методы *generate* и *generateUnorderedSet* дополнительно можно передать lambda-функцию с набором свойств, которыми
должны обладать получаемые квазигруппы.
...
Метод *transform* класса *PFQuasigroupPermutationTransformer* можно использовать для преобразования квазигруппы,
созданной на основе
правильного семейства функций в другую квазигруппу с применением перестановочных конструкций на индексы переменных и
функций семейства

```c++
Quasigroup::PFQuasigroupFactory factory(2, 3);

const auto q = factory.create();
const auto transformedQ = Quasigroup::PFQuasigroupPermutationTransformer::transform(q);
...
Класс *ExperimentalReport* записывается в консоль и файл в формате json, для удобства дальнейшей визуализации
результатов.
