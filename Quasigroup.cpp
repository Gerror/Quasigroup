	...
	bool Quasigroup::IsAffine()
	{
		...
		*/
	    ...
	    for(int i = 0; i < order; i++)
	    {
	        ...
	        {
	            if(latinSquare[i][j] !=
	               tempMatrix[latinSquare[0][0] + tempMatrix[latinSquare[beta][j] + latinSquare[i][alpha] * order] * order])
	                return false;
	        }
	    }
	...
	bool Quasigroup::TrueLatinSquare()
	{
		...
		for (int i = 0; i < order; i++)
		{
			for (int j = 0; j < order; j++)
			{
				for (int m = 0; m < order; m++)
				{
					if (m != i && m != j && (latinSquare[i][m] == latinSquare[i][j] || latinSquare[m][j] == latinSquare[i][j]))
						return false;
				}
			}
		}
