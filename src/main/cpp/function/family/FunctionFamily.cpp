...
FunctionFamily::FunctionFamily(const FunctionFamily &ff)
    : FunctionFamily(ff.k, ff.n, ff.numberOfFunctions, ff.functionSize) {
...
FunctionFamily::FunctionFamily(const int k, const int n,
                               const int numberOfFunctions,
                               const int functionSize, std::ifstream &input)
...
int FunctionFamily::getK() const { return k; }

int FunctionFamily::getN() const { return n; }

int FunctionFamily::getFunctionValue(const int varValue,
                                     const int functionNumber) const {
  return functionFamily[functionNumber][varValue];
}

int FunctionFamily::getNumberOfFunctions() const { return numberOfFunctions; }

int FunctionFamily::getFunctionSize() const { return functionSize; }

std::ostream &operator<<(std::ostream &out, const FunctionFamily &ff) {
  for (int functionNumber = 0; functionNumber < ff.getNumberOfFunctions();
       functionNumber++) {
...
bool operator==(const FunctionFamily &ff1, const FunctionFamily &ff2) {
  if (ff1.getNumberOfFunctions() != ff2.getNumberOfFunctions() ||
      ff1.getFunctionSize() != ff2.getFunctionSize()) {
    return false;
  }

  const int numberOfFunctions = ff1.getNumberOfFunctions();
  const int functionSize = ff1.getFunctionSize();

  for (int functionNumber = 0; functionNumber < numberOfFunctions;
       functionNumber++) {
    for (int arg = 0; arg < functionSize; arg++) {
      if (ff1.getFunctionValue(arg, functionNumber) !=
          ff2.getFunctionValue(arg, functionNumber)) {
        return false;
      }
    }
  }
