...
AbelianGroupAutomorphism::AbelianGroupAutomorphism(
    AbelianGroup *group, const unsigned long long int seed)
...
void AbelianGroupAutomorphism::generate() {
  ...
  int i = 0;
  for (const auto componentOrder : group->getCyclicGroupsOrders()) {
    std::vector<int> variants;
    for (int j = 1; j < componentOrder; j++) {
      if (const auto gcd = std::gcd(j, componentOrder); gcd == 1) {
        variants.push_back(j);
      }
    }
    const int eImage = variants[mersenne() % variants.size()];
  ...
  for (int x = 0; x < group->getOrder(); x++) {
    ...
    for (int j = 0; j < xComponents.size(); j++) {
      xComponents[j] = mod(xComponents[j] * generatingSetImages[j],
                           group->getCyclicGroupsOrders()[j]);
    }
    images.push_back(group->convertGroupComponentsToElement(xComponents));
  }
}

int AbelianGroupAutomorphism::getImage(const int x) const { return images[x]; }

bool AbelianGroupAutomorphism::isCorrectHomomorphism() const {
  for (int x = 0; x < group->getOrder(); x++) {
    for (int y = 0; y < group->getOrder(); y++) {
      if (getImage(group->getProduct(x, y)) !=
          group->getProduct(getImage(x), getImage(y))) {
        return false;
      }
    }
  }
